...
p <- seq_len(floor(length(bn.higher.samples) / 2)) %>%
    lapply(function(i) {
        ...
        doMAPlot(logcpm, s1, s2) +
            labs(title=sprintf("MA plot of %s-radius promoters", params$promoter_radius),
...
p <- seq_len(floor(length(bn.higher.samples) / 2)) %>%
    lapply(function(i) {
        ...
        doMAPlot(logcpm, s1, s2, AveLogCPM=AveLogCPM,
            ...
            labs(title=sprintf("MA plot of %s-radius promoters with common A scale", params$promoter_radius),
                 subtitle=sprintf("%s vs %s", colnames(dge)[s1], colnames(dge)[s2]))
    })
ggprint(p)
```
...
p <- seq_len(floor(length(bn.higher.samples) / 2)) %>%
    lapply(function(i) {
        ...
        doMAPlot(logcpm.loess, s1, s2, linedata = NULL) +
            ...
            labs(title=sprintf("MA plot of %s-radius promoters, loess normalized", params$promoter_radius),
                 subtitle=sprintf("%s vs %s", colnames(dge)[s1], colnames(dge)[s2]))
    })
ggprint(p)
```
...
p <- seq_len(floor(length(bn.higher.samples) / 2)) %>%
    lapply(function(i) {
        ...
        doMAPlot(bcdata$PromoterNorm$E, s1, s2, linedata = NULL) +
            geom_hline(yintercept=0) + 
            labs(title=sprintf("MA plot of %s-radius promoters, peak-normalized & SVA-corrected", params$promoter_radius),
                 subtitle=sprintf("%s vs %s", colnames(dge)[s1], colnames(dge)[s2]))
    })
ggprint(p)
```
