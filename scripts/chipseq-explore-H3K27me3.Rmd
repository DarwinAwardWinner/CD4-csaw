...
sexpfile <-
    here("saved_data",
         glue_data(params, "chipseq-counts_{window_size}-windows_{fragment_length}-reads_{histone_mark}.RDS"))
bigbin.sexpfile <-
    here("saved_data",
...
With the background normalization, we can clearly see that the first surrogate variable becomes a proxy for ChIP efficiency. This is a hint that efficiency normalization may be correct for this data set. Beyond that, it seems that some of the surrogate variables may be capturing the donor effects, while others seem to be confounded with the time point. This confounding is a potential concern, and we should be on the lookout for possible consequences of this in downstream analyses.
...
The peak-based normalizations and the loess normalization produce the smallest BCV estimates, but the overall variability in this data set is fairly low for all normalizations. Based on these plots, there is no clear reason to prefer one normalization over the others.
...
Not only are the BCV values higher without SVA, the BCV also trends upward with abundance rather than downward as is typical. These BCV plots are indicative of the likely presence of unmodeled batch effects and underscore the importance of SVA for this data set. In any case, we can quantify the effect of SVA by looking at the distribution of changes in BCV when the surrogate variables are added to the model.
...
We can see that the median change in BCV is around -0.1 for the peak-based normalizations and -0.12 for the other normalizations. Next, we examine the effect of each normalization on the MDS plot.
...
```{r mds_plot_multidim}
xlims <- range(unlist(mdstabs$PeakNorm[glue("Dim{1:4}")]))
ylims <- range(unlist(mdstabs$PeakNorm[glue("Dim{2:5}")]))
pbase <- ggmds(mdstabs$PeakNorm) +
...
```{r mds_var_explained}
mds_values <- mdstabs$PeakNorm %>% dplyr::select(dplyr::matches("^Dim[0-9]+$")) %>% as.matrix
...
elists <- bplapply(designs, voomWithOffset, counts = dges$PeakHANorm)
...
```{r selectModel}
sm <- selectModelParallel(voomWithOffset(dges$PeakHANorm, designs[[1]]), designs, criterion = "bic")
