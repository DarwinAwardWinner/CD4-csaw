...
FactorsScatterPairs(mofa, factors=factors_of_interest, color_by="time_point", shape_by = "cell_type")
```
...
We are now ready to conduct feature set enrichment tests for the factors of interest. We also run the same test with the membership matrix inverted to test for feature set depletion. 

```{r fsea_run}
fsea_results <- lapply(names(overlap_sets) %>% setdiff("H3K4me3") %>% set_names, function(view) {
    message("")
...
rnaseq_data$MOFAPredict <- predict(mofa, view="RNA", factors=c("1", "5", "6"))$RNA
