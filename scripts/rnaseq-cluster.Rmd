...
sexps <- bplapply(sexpfiles, readRDS)
for (i in names(sexps)) {
    colnames(sexps[[i]]) <- colData(sexps[[i]])$SampleName
    colnames(bigbin_sexps[[i]]) <- colData(bigbin_sexps[[i]])$SampleName
    # Ensure identical column order
    bigbin_sexps[[i]] %<>% .[,colnames(sexps[[i]])]
    assert_that(all(colnames(sexps[[i]]) == colnames(bigbin_sexps[[i]])))
}
```
...
elist.cb$E <- ComBat(elist.cb$E, batch = sample.table$batch, mod = design.cb, par.prior = ifelse(interactive(), TRUE, FALSE))
```

```{r rlog_combat}
rlogcpm.cb <- rlogcpm
assay(rlogcpm.cb) <-ComBat(assay(rlogcpm), batch = sample.table$batch, mod = design.cb, par.prior = ifelse(interactive(), TRUE, FALSE))
```
