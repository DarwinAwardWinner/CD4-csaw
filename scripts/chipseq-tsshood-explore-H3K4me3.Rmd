...
bgnf %<-% normOffsets(bigbin_sexp, type = "scaling", weighted = FALSE, se.out = TRUE)
pnf %<-% normOffsets(nhood_sexp, type = "scaling", weighted = TRUE, se.out = TRUE)
wnf %<-% normOffsets(sexp, type = "scaling", weighted = TRUE, se.out = TRUE)
loff %<-% { normOffsets(nhood_sexp, type = "loess", se.out = TRUE) + mean(getOffset(dge)) }
...
outliers <- apply(all_sv, 2, . %>% abs %>% which.max) %>% unique
assert_that(length(outliers) == 2)
good_samples <- colnames(sexp)[-outliers]
