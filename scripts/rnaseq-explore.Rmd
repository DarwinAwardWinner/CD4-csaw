...
params:
  dataset:
    value: star_hg38.analysisSet_ensembl.85
  basedir:
...
Lastly, we define a convenience function for alternating voom and duplicateCorrelation.

```{r voom_and_dupcor_function}
voomWithDuplicateCorrelation <- function(counts, design = NULL, plot = FALSE, block = NULL, trim = 0.15, voom.fun=voom, dupCor.fun=duplicateCorrelation, initial.correlation=0, niter=2, tol=1e-6, verbose=TRUE, ...) {
    assert_that(niter >= 1)
...
elist.w <- voomWithDuplicateCorrelation(dge, design.NoTime, block=sample.table$batch, niter=10, voom.fun=voomWithQualityWeightsAndOffset)
```
