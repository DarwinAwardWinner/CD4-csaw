...
library(doParallel)
ncores <- min(2, getOption("mc.cores", default=parallel::detectCores(logical = FALSE)))
...
We compute the MDS coordinates after subtracting the batch effects of both FRiP and donor_id. The result is a plot showing the variation attributable to sources other than those batch effects. 

```{r compute_mds_normtest}
batch.cols <- str_detect(colnames(design), "^FRiP\\.NS|^D\\d+\\.vs\\.D\\d+$")
bcdata <- dges %>% bplapply(. %>% voomWithOffset(design=design) %>%
                                assign_into(.$E, removeBatchEffect(.$E, design = design[,!batch.cols], covariates=design[,batch.cols])))
# bcdata <- dges %>% bplapply(. %>% cpmWithOffset(log=TRUE, prior.count=0.5) %>% 
#                                 removeBatchEffect(design = design[,!batch.cols], covariates=design[,batch.cols]))
mdsdist <- bcdata %>% bplapply(. %>% {suppressPlot(plotMDS(.))} %$% distance.matrix %>% as.dist)
...
From the MDS plots, we can see that the two variants on peak normalization and the background normalization all produce similar reasonable-looking results, with good separation between time points and marginal separation between cell types, consistent with previous analysis of this data. The lowess normalization also separates the time points, well, but it also seems to cause some samples, especially those in Day 1, to become outliers, which means that the lowess method may be normalizing too aggressively and distorting the data as a result.

Based on this, the peak-based normalization seems like the best fit for this data set.
