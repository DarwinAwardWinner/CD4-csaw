...
TODO REWRITE From the MDS plots, we can see that the two variants on peak normalization and the background normalization all produce similar reasonable-looking results, with good separation between time points and marginal separation between cell types, consistent with previous analysis of this data. The lowess normalization also separates the time points, well, but it also seems to cause some samples, especially those in Day 1, to become outliers, which means that the lowess method may be normalizing too aggressively and distorting the data as a result.
