...
bcv_limits <- disptable %>% select(BCV_SVA, BCV_NoSVA) %>% unlist %>% quantile(c(.001, .999)) %>%
    ifelse(. < 0.1, 0, .)
ggplot(disptable) + 
    aes(x=NormType, y=BCV_NoSVA) + 
    geom_hline(yintercept = 0, linetype="dashed", alpha=0.5) +
    geom_violin(scale = "area", draw_quantiles = (1:3)/4) +
    coord_cartesian(ylim=bcv_limits) +
...
The peak-based normalization produce the cleanest, most interpretable MDS plots by far, with both naive and memory cells tracing parallel arcs over time. The background normalization is much more muddled, and the loess normalization seems to be riddled with outliers, even after subtracting surrogate variables. These results definitely favor peak-based normalization with SVA as the best modelling strategy for this data.
