...
message(sprintf("Excluding %i out of %i windows (%.3g%%) with average count below %s.",
...
All normalizations produce very similar BCV plots. The largest difference is the slightly higher prior d.f. for the loess normalization. There is no clear preference for one normalization method based on these plots.
...
All normalization methods produce similar looking plots for dimensions 1 and 2 except for the peak-based normalization, which seems to look very different. However, looking at all three dimensions together, it's clear that if dimensions 2 and 3 were swapped for this normalization, the plots would look very similar. Compare the following plots, in which dimensions 2 and 3 have been swwapped:

```{r mds_hanorm_swapdim}
ggprint(p12$PeakNorm + aes(x=Dim1, y=Dim3) +
        labs(title="MDS Principal Coordinates 1 & 3"))
ggprint(p12$PeakNorm + aes(x=Dim3, y=Dim2) +
        labs(title="MDS Principal Coordinates 3 & 2"))
```

This swapping of dimensions probably occurred because dimensions 2 & 3 both account for similar proportions of the total variance, so a slight change in normalization was enough to push dimension 3 slightly ahead of dimension 2, causing them to be swapped when ordered by fraction of variance explained. (This is a useful reminder not to assume that the first 2 principal coordinates tell the full story of the data.) 

After accounting for this discrapancy, all 4 normalizations produce similar results. Again, there is little reason to prefer one over the others based on the MDS plots. The "high abundance windows within peaks" normalization method is the simplest to implement, since it is computed from only the count data of the windows being analyzed and does not rely on external data, such as filtered-out low-count windows or a separate count table for large bins. Hence, this normalization method will be used going forward.
...
Without subtracting the effects of FRiP, the results are somewhat different. The same overall patterns of variation are still discernable, but less clearly so compared to the MDS plots with FRiP effects subtracted. From this, we conclude that adding FRiP as a covariate is justified.
