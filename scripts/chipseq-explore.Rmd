...
# NOTE: Assumes windows that exactly tile the genome (no overlaps, no gaps).
...
p <- list(
    ...
    Violin=ggplot(adata) +
        ...
        geom_violin() +
        labs(title="Violing plot of average window logCPM values",
...
sample.table <- colData(sexp) %>%
    as.data.frame %>%
    mutate(group=interaction(cell_type, time_point, sep="", lex.order = TRUE) %>%
               `levels<-`(str_replace_all(levels(.), "Day", "D"))) %>%
    autoFactorize
# Use sum-to-zero coding matrices
