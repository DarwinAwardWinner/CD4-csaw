...
We select only genes with different expression patterns in Naive and Memory cells.

```{r rnaseq_diffexp}
design_diffexp <- model.matrix(~ cell_type*time_point - cell_type + donor_id, sample_table)
NvM_diff_coefs <- colnames(design_diffexp) %>% .[str_detect(., "cell_type")]
fit <- lmFit(elist.cb, design_diffexp) %>% eBayes(robust = TRUE)
cfit <- contrasts.fit(fit, coef = NvM_diff_coefs) %>% eBayes(robust = TRUE)
ttab <- topTable(cfit, n = Inf, sort.by = "none")
rnaseq_NvM_diff <- ttab %$% ENSEMBL[adj.P.Val <= 0.1]
```
...
logCPM <- assay(rlogcpm.cb) %>% .[rownames(.) %in% rnaseq_NvM_diff,] %>%
    subtractCoefs(design, coefsToSubtract = donor_coefs)
```
...
km_centers_clamped <- km$centers
km_centers_clamped[] %<>% {clamp_trans(-3, 3)$transform(.)}
pheatmap(km_centers_clamped, cluster_rows = FALSE, cluster_cols = FALSE,
...
```{r kmeans_trajectories, fig.height = 12, fig.width = 12}
group_logcpm <- elist.cb$E %>% as_tibble(rownames = "GeneID") %>%
    gather("SampleName", "logCPM", -GeneID) %>% 
    inner_join(sample_table, by = "SampleName") %>% 
    group_by(GeneID) %>% mutate(logCPM = logCPM - mean(logCPM)) %>% ungroup %>%
    group_by(GeneID, group, time_point, cell_type) %>% 
    summarize(logCPM = mean(logCPM)) %>%
    mutate(Cluster = str_c("C", km$cluster[GeneID])) %>% 
    filter(!is.na(Cluster))
ggplot(group_logcpm) + 
    facet_grid(rows = vars(Cluster), cols = vars(cell_type)) +
    aes(x = time_point, y = logCPM, group = GeneID) +
...
for (mark in names(chipseq_data)) {
    ...
    for (grp in levels(chipseq_data[[mark]]$sample_table$group)) {
        p[[mark]][[grp]] <-
            ggplot(group_logcpm_centered %>% filter(Group == grp)) +
