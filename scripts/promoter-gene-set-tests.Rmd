...
results.tables <- diffmod$results.tables %>%
    Filter(f=. %>% {"logFC" %in% colnames(.)})
```
...
camera.results <- lapply(all_collections, function(coll) {
    bplapply(results.tables, function(rt) {
        cameraPR(statistic=rt %$% {F * sign(logFC)}, index=ids2indices(coll, rt$GeneID)) %>%
