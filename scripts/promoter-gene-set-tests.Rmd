...
We begin by loading the saved data from the ChIP-seq promoter differential modification tests. We will use the computed t statistics for all the single-contrast tests. (ANOVA-like tests are skipped.)
...
for (i in names(camera.results)) {
    ...
    write.xlsx(sig.results, here("results", "ChIP-seq", glue("CAMERA-results-{params$histone_mark}_{params$promoter_radius}-promoter-{i}.xlsx")))
}
saveRDS(camera.results, here("saved_data", glue_data(params, "CAMERA-results-{histone_mark}_{promoter_radius}-promoter.RDS")))
```
