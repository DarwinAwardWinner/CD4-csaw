...
knitr::opts_chunk$set(echo = TRUE, retina=2, cache=TRUE, autodep=TRUE,
                      ...
                      cache.path = paste0(
                          here("cache", "rnaseq-diffexp", params$dataset),
                          .Platform$file.sep))
```
...
saveRDS(results.tables, here('saved_data', 'RNA-seq', sprintf('%s-diffexp-tables.RDS', params$dataset)))
save.image(here('saved_data', 'RNA-seq', sprintf('%s-diffexp.rda', params$dataset)))
write.xlsx(filtered.results.tables, here("results", "RNA-seq",sprintf('%s-diffexp.xlsx', params$dataset)))
```
