...
All 4 normalization methods produce similar looking plots for all 3 dimensions, with the samples from each time point clustering together and each donor progressing through the time points in a roughly cyclical fashion in dimensions 1 and 2. Dimension 3 does not obviously correlate with any of the known experimental variables. The loess normalization seems like it causes some previously well-behaved samples to become outliers, which is not desirable. Based on the BCV and MDS plots, any of the proposed scaling normalizations is appropriate. The "high abundance windows within peaks" normalization method is the simplest to implement, since it is computed from only the count data of the windows being analyzed and does not rely on external data, such as filtered-out low-count windows or a separate count table for large bins. Hence, this normalization method will be used going forward.
...
Without subtracting the effects of FRiP, the cyclic patterns observed previously are still discernable in some normalizations, but even then, the time points are now substantially overlapped instead of cleanly separated. From this, we conclude that adding FRiP as a covariate is justified.
